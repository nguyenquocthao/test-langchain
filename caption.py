...
class ImageCaptionLoader(BaseLoader):
    ...
    def _get_captions_and_metadata(
        self, model: Any, processor: Any, path_image
    ) -> Tuple[str, dict]:
        """
        Helper function for getting the captions and metadata of an image
        """
        # print(68, '_get_captions_and_metadata', model, processor, path_image)
        # try:
        #     from PIL import Image
        # except ImportError:
        #     raise ValueError(
        ...
        try:
            if isinstance(path_image, str) and (path_image.startswith("http://") or path_image.startswith("https://")):
